...
Кнопка 🏆 показывает личные показатели и локальную доску лучших результатов —
они работают без регистрации и сети. Для общей аналитики и глобального рейтинга
подготовлен Cloudflare Worker + D1 в [`backend/`](backend/README.md): после его
публикации добавь три URL (`ANALYTICS_ENDPOINT`, `DASHBOARD_ENDPOINT`,
`LEADERBOARD_ENDPOINT`) в `config.js`. До этого интерфейс честно помечает данные
как локальные и не подставляет вымышленные результаты.
