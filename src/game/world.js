...
export class World {
  ...
  _drawRidge(ctx, W, horizon, pts, off, color, alpha) {
    // Нулевая ширина роняла ВЕСЬ игровой цикл: sx / W даёт -0/0 = NaN, дальше
    // i0 = Math.floor(NaN) % n = NaN и pts[NaN].h бросает TypeError. Исключение
    // прилетало из rAF-кадра, то есть цикл умирал молча и игрок видел застывший
    // экран. Нулевой вьюпорт реален на первом кадре в Telegram WebView, пока идёт
    // согласование viewport — гряды тут просто нечего рисовать, выходим тихо.
