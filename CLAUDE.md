...
npm run shots:diff # рендер превью + попиксельное сравнение с версией из git
```
**Любую правку рендера гоняй через `npm run shots:diff`.** Оптимизация обязана
быть визуально нейтральной; если превью уехало за допуск — это не оптимизация.
...
- **Игра — Telegram Mini App.** Игрок опознаётся подписанным `initData` и
  ...
  не «приз дойдёт».
- **Награда за манёвр не должна зависеть только от факта ввода.** Near-miss
  начислялся за «недавно двигался + препятствие рядом», и непрерывное дёрганье
  полос засчитывало его почти на каждой колонне: комбо у потолка, биты ×3,
  49.6 очк/м против честных 20.2 (прод 2026-08-07, один игрок из 146 занял так
  первое место с отрывом 2.6×). Лечится анти-дребезгом по частоте манёвров
  (`CONFIG.NEARMISS_MAX_MOVES_SEC`, чистая `laneThrash()` в `stats.js`). Любое
  новое правило «за активность» проверяй вопросом: что будет, если игрок начнёт
  нажимать это непрерывно? И сверяй экономику в удельных величинах — очки за
  метр вскрывают сломанное правило там, где абсолютный счёт выглядит как
  мастерство. Разбор и обоснование, почему `RULESET_VERSION` там НЕ поднимался:
...
- **Дорог не DPR, а `shadowBlur` и градиенты на кадр.** DPR уже ограничен
  сверху (`min(devicePixelRatio, cap)` в `setupCanvas`), поэтому на тире 0 кадр
  это ~0.37 Мпикс — кадром в 126 мс число пикселей не объяснить. `dpr=3` в
  телеметрии — маркер класса устройства, а не причина. Статичные части сцены
  (вышки, линия горизонта, ободок солнца) кэшируются в оффскрин, градиенты — по
  ключу с `ctx`. Слой ОБЯЗАН строиться в пикселях устройства (`layerScale`/
  `makeLayer`), иначе блит растянет неон в мыло. Разбор кадра по секциям есть в
  `run_summary` (`secWorldMs`/`secEntitiesMs`/`secPostMs`) — оптимизируй по нему,
  а не по догадкам. Подробности и урок про недостоверные микробенчмарки:
  `docs/solutions/2026-08-06-frame-cost-is-shadowblur-not-dpr.md`.
...
- **Игра сама предлагает лёгкий режим** (`shouldOfferLite()` в `quality.js`,
  показ на game over) — прод-данные первого дня: настройку находят 4.6%
  сессий, а просадка есть у ~46% забегов. Показ ограничен `CONFIG.QUALITY.HINT_MAX`
  раз за устройство; любой явный ответ («включить» или «не надо») закрывает
  подсказки навсегда через `Settings.graphicsTouched`. Подсказка живёт СТРОГО
  на экране game over, не в игровом кадре — тап мимо кнопки в забеге стоил бы
  игроку полосы движения. Событие `lite_hint` — в белых списках `server.js` и
  `worker.js`. Разбор данных и пороги: `docs/solutions/2026-08-05-lite-graphics-mode.md`.
